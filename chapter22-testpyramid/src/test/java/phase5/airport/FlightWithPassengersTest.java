...
import phase4.airport.DistancesManager;
import phase4.airport.Flight;
import phase4.airport.Passenger;
import phase5.airport.annotations.FlightNumber;
...
//    결과적으로는 테스트는 매우 간단하고 관리하기 쉬워지는데 현재는 컨테이너에 Passenger와 Flight 클래스가 포함되어 있다
    public static JavaArchive createDeployment() {
        return ShrinkWrap.create(JavaArchive.class)
                .addClasses(phase4.airport.Passenger.class, phase4.airport.Flight.class, FlightBuilderUtil.class, phase4.airport.DistancesManager.class)
...
phase4.airport.Flight flight;

    @Inject
    @FlightNumber(number = "AA1235")
    phase4.airport.Flight flight2;
...
//    private static Map<Passenger, Integer> passengersPointsMap = new HashMap<>();
    private static Map<phase3.airport.Passenger, Integer> passengersDistancesMap = new HashMap<>();
...
//        passengersPointsMap.put(new Passenger("123-45-6799", "Bethany King", "US"), 630);
        passengersDistancesMap.put(new phase3.airport.Passenger("900-45-6809", "Susan Todd", "GB"), 2100);
        passengersDistancesMap.put(new phase3.airport.Passenger("900-45-6797", "Harry Christensen", "GB"), 4200);
        passengersDistancesMap.put(new phase3.airport.Passenger("123-45-6799", "Bethany King", "US"), 6300);
    }
...
//    그 다음 @Test 어노테이션이 붙은 메서드가 컨테이너 환경 내에서 수행된다
    ...
    public void testNumberOfSeatsCannotBeExceeded() throws IOException {
        ...
        flight.addPassenger(new phase4.airport.Passenger("124-56-7890", "Michael Johnson", "US"));
    }

    @Test
    public void testAddRemovePassengers() throws IOException {
        flight.setSeats(51);
        phase4.airport.Passenger additionalPassenger = new phase4.airport.Passenger("124-56-7890", "Michael Johnson", "US");
...
//        when(distancesManager.getPassengersDistancesMap()).thenReturn(passengersDistancesMap);
        ...
        for(phase4.airport.Passenger passenger : flight.getPassengers()){
            distancesManager.addDistance(passenger, flight.getDistance());
        }

        for(phase4.airport.Passenger passenger : flight2.getPassengers()){
            distancesManager.addDistance(passenger, flight2.getDistance());
        }

        for(phase4.airport.Passenger passenger : flight3.getPassengers()){
            distancesManager.addDistance(passenger, flight3.getDistance());
        }

        distancesManager.calculateGivenPoints();

        assertEquals(210, distancesManager.getPassengersPointsMap().get(new phase4.airport.Passenger("900-45-6809", "Susan Todd", "GB")).longValue());
        assertEquals(420, distancesManager.getPassengersPointsMap().get(new phase4.airport.Passenger("900-45-6797", "Harry Christensen", "GB")).longValue());
