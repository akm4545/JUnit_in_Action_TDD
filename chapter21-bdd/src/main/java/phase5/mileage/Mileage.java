package phase5.mileage;

import phase5.airport.Passenger;
...
public class Mileage {
    ...
    public void calculateGivenPoints() {
        for(Passenger passenger : passengersMileageMap.keySet()){
            if(passenger.isVip()){
                passengersMileageMap.put(passenger, passengersMileageMap.get(passenger) / VIP_FACTOR);
            }else{
                passengersMileageMap.put(passenger, passengersMileageMap.get(passenger) / REGULAR_FACTOR);
            }
        }
    }
}
